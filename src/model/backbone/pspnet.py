...
class PSPNet(nn.Module):
    ...
    def inner_loop(self, f_s, s_label):
        ...
        # [ n_shots, img_size, img_size]
        s_label_arr = s_label.cpu().numpy().copy()
        back_pix = np.where(s_label_arr == 0)
        target_pix = np.where(s_label_arr == 1)
        # bg的weight: num of gf pixels
...
def parse_param_coscls(cls_type):
    ...
    print('weight norm {}, bias {}, temp {}'.format(
        weight_norm_dt[cls_type[0]], bias_dt[cls_type[1]], temp_dt[cls_type[2]]))
