...
def get_param_groups(model, args):
    ...
    if args.lr_mode == 'none':
        ...
        if model.encoder.arch.find('psp') != -1:
            freeze_params = concat_params(
                [model.encoder.pspnet.__getattr__(f'layer{i}') for i in range(5)])
        elif model.encoder.arch.find('pvt') != -1:
            freeze_params = model.encoder.pvt.parameters()
        else:
        ...
        if model.encoder.arch.find('psp') != -1:
            decay_params = concat_params(
                [model.encoder.pspnet.__getattr__(f'layer{i}') for i in range(5)])
        elif model.encoder.arch.find('pvt') != -1:
            decay_params = model.encoder.pvt.parameters()
        else:
            decay_params = model.encoder.parameters()
        decay_ids = list(map(id, decay_params))
        other_params = filter(lambda p: id(
            p) not in decay_ids, model.parameters())
        return [
        ...
        if model.encoder.arch.find('vit') != -1:
            param_groups.append(
                {'params': model.encoder.vit.norm.parameters(), 'lr': lr, 'weight_decay': 0.0})
            for i in range(args.encoder_blocks-1, -1, -1):
                decayed_lr = lr * (args.lr_decay ** (args.encoder_blocks - i))
                param_groups += split_params(
                    model.encoder.vit.blocks[i], decayed_lr)
            decayed_lr = lr * (args.lr_decay ** (args.encoder_blocks + 1))
            param_groups += split_params(
                model.encoder.vit.patch_embed, decayed_lr)
        elif model.encoder.arch.find('psp') != -1:
            param_groups.append(
                {'params': model.encoder.pspnet.ppm.parameters(), 'lr': lr})
            param_groups.append(
                {'params': model.encoder.pspnet.bottleneck.parameters(), 'lr': lr})
            for i in range(args.encoder_blocks-1, -1, -1):
                decayed_lr = lr * (args.lr_decay ** (args.encoder_blocks - i))
                param_groups += split_params(
                    model.encoder.pspnet[i], decayed_lr)
        else:
            for i in range(args.encoder_blocks-1, -1, -1):
                decayed_lr = lr * (args.lr_decay ** (args.encoder_blocks - i))
                param_groups += split_params(
                    model.encoder.resnet[i], decayed_lr)
        return param_groups
