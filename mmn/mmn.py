...
from model.utils import LOSS_DICT
...
class MMNModel(nn.Module):
    ...
    def inner_loop(self, f_s, label_s):
        ...
        optimizer = torch.optim.SGD(
            self.classifier.parameters(), lr=self.args.lr_cls)
    ...
    def forward(self, backbone, img_s, img_q, label_s, label_q, use_amp=False):
        ...
        with amp.autocast(enabled=use_amp):
            ...
            for k in range(self.shot):
                ...
                # [1, 512, h, w]
                att_f_q.append(self.trans(mf_q, curr_mf_s, curr_f_s))
            # [k, 512, h, w] => [1, 512, h, w]
            att_f_q = torch.cat(att_f_q, dim=0).mean(dim=0, keepdim=True)
