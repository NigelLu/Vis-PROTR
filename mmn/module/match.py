...
from model.utils import get_corr
from mmn.module.conv4d import CenterPivotConv4d, Conv4d
from mmn.module.base import (
...
def MutualMatching_chn(corr4d):
    ...
    corr4d_B = corr4d.view(batch_size, fs1*fs2, fs3,
                           fs4)  # [batch_idx,k_A,i_B,j_B]
...
class NeighConsensus(torch.nn.Module):
    def __init__(self, kernel_sizes=[3, 3, 3], channels=[10, 10, 1], symmetric_mode=True, conv='cv4', in_channel=1):
        ...
        for i in range(num_layers):
            ...
            nn_modules.append(conv4_dt[conv](in_channels=ch_in, out_channels=ch_out, kernel_size=(
                k_size,)*4, padding=(1,)*4, bias=True))
    ...
    def forward(self, x):
        if self.symmetric_mode:
            ...
            x = self.conv(x)+self.conv(x.permute(0, 1, 4, 5, 2, 3)
                                       ).permute(0, 1, 4, 5, 2, 3)
...
class MatchNet(nn.Module):
    def __init__(self, temp=3.0, cv_type='red', in_channel=1, sce=False, cyc=False, sym_mode=True, cv_kernels=[3, 3, 3], cv_channels=[10, 10, 1]):
        ...
        if self.sce:
            sce_ksz = 25
            self.SpatialContextEncoder = SpatialContextEncoder(
                kernel_size=sce_ksz, input_dim=sce_ksz * sce_ksz + 2048, hidden_dim=2048)
        if self.cyc:
            self.ass_drop = nn.Dropout(0.1)

        self.NeighConsensus = NeighConsensus(
            kernel_sizes=cv_kernels, channels=cv_channels, symmetric_mode=sym_mode, conv=cv_type, in_channel=in_channel)

    # ig_mask [1, 3600]
    def forward(self, fq_fea, fs_fea, v, s_mask=None, ig_mask=None, ret_corr=False, use_cyc=False, ret_cyc=False):
        ...
        if self.cyc and use_cyc:
            # positive means inconsistent [B, N_s]
            inconsistent_mask = self.run_cyc(corr2d, s_mask)
        ...
        # [B, 512, N_s] * [B, N_s, N_q] -> [1, 512, N_q]
        weighted_v = torch.bmm(v, attn.permute(0, 2, 1))
    ...
    def run_cyc(self, corr2d, s_mask):
        ...
        if s_mask is not None:  # [B, 1, 60, 60]
            ...
            re_map_idx = torch.gather(
                q2k_sim_idx, 1, k2q_sim_idx)  # [B, n_s]  k->q->k
            re_map_mask = torch.gather(s_mask, 1, re_map_idx)       # [B, n_s]

            # [B, n_s], True means matched position in supp
            association = (s_mask == re_map_mask).to(corr2d.device)
...
class CHMLearner(nn.Module):
    ...
    def __init__(self, ktype, feat_dim, temp=20.0):
        ...
        self.scales = [0.5, 1, 2]
        self.conv2ds = nn.ModuleList([nn.Conv2d(
            feat_dim, feat_dim // 4, kernel_size=3, padding=1, bias=False) for _ in self.scales])
    ...
    def forward(self, src_feat, trg_feat, v, ig_mask=None, ret_corr=False):
        ...
        corr = Correlation.build_correlation6d(
            src_feat, trg_feat, self.scales, self.conv2ds).unsqueeze(1)   # [1, 1, 3, 3, 30, 30, 30 30]
        ...
        # Scale-space maxpool
        # max pool along scale  [1, 30, 30, 30, 30]
        ...
        corr = Correlation.mutual_nn_filter(corr.view(
            bsz, corr.size(-1) ** 2, corr.size(-1) ** 2).contiguous())  # [1, 3600, 3600]
