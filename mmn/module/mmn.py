...
from mmn.module.match import MatchNet
...
class MMN(nn.Module):
    def __init__(self, args):
        ...
        self.rmid = args.rmid if type(args.rmid) == list else [
            2, 3, 4]           # List[int]
    ...
    def forward(self, mf_q, mf_s, f_s):
        ...
        for idx in self.rmid:
            # [B, C, H, W]
            fq_fea, fs_fea = mf_q[idx][0], mf_s[idx][0]
            if self.red_dim:
        ...
        # [B, L, H, W, H, W]
        corr4d = torch.cat(corr_lst, dim=1)
        if self.agg == 'sum':
            # [B, 1, H, W, H, W]
            corr4d = torch.sum(corr4d, dim=1, keepdim=True)
...
class WeightAverage(nn.Module):
    ...
    def forward(self, x):
        ...
        phi = phi.contiguous().view(batch_size, self.R, self.R, self.c_out,
                                    h, w)                           # K
        # BS, h, w, c, R, R                                            # K
        phi = phi.permute(0, 4, 5, 3, 1, 2)
        theta = theta.permute(0, 2, 3, 1).contiguous().view(
            batch_size, h, w, self.c_out)   # BS, h, w, c   # Q
        # Q
        theta_dim = theta

        cos_sim = self.CosSimLayer(
            phi, theta_dim[:, :, :, :, None, None])  # BS, h, w, c, R, R
